...
def kbo_games():
    ...
    while d<=END:
        idx+=1
        print(f"   KBO {idx}/{total}일 수집: {d}", flush=True)
...
def npb_games():
    ...
    while d<=END:
        idx+=1
        print(f"   NPB {idx}/{total}일 수집: {d}", flush=True)
...
def main():
    allres={};details=[]
    print("▶ [1/3] MLB 최근 30일 데이터 수집 시작...", flush=True)
    mg=mlb_games()
    print("   MLB 경기결과 수집 완료:",len(mg),"경기", flush=True)
    mc=mlb_current()
    print("   MLB 경기직전 기록 역산/분석중...", flush=True)
    mr,md=evaluate("MLB",mg,mc);allres["MLB"]=mr;details+=md
    print("✅ MLB 완료", flush=True)

    print("▶ [2/3] KBO 최근 30일 데이터 수집 시작...", flush=True)
    kg=kbo_games()
    print("   KBO 경기결과 수집 완료:",len(kg),"경기", flush=True)
    kc=kbo_current()
    print("   KBO 경기직전 기록 역산/분석중...", flush=True)
    kr,kd=evaluate("KBO",kg,kc);allres["KBO"]=kr;details+=kd
    print("✅ KBO 완료", flush=True)

    print("▶ [3/3] NPB 최근 30일 데이터 수집 시작...", flush=True)
    ng=npb_games()
    print("   NPB 경기결과 수집 완료:",len(ng),"경기", flush=True)
    nc=npb_current()
    print("   NPB 경기직전 기록 역산/분석중...", flush=True)
    nr,nd=evaluate("NPB",ng,nc);allres["NPB"]=nr;details+=nd
    print("✅ NPB 완료", flush=True)
...
PYTHONUNBUFFERED=1 python3 -u weight_backtest.py | tee 가중치30일결과.txt
